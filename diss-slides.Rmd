...
.pull-left[<img src="figs/mouse_on_chips.png" width="200" height="150" />]

.pull-right[.small[Photo by Alan Attie]]
...
# Benefits of a pleiotropy test for multiparental populations
...
- `r Citet(myBib, "keller2018genetic")` measured pancreatic islet gene expression levels in 378 Diversity Outbred mice
...
- Examine power when expression trait QTL locations are known
...
```{r, out.height = "500px"}
include_graphics("figs/lrt-v-middle-of-gene-slides-1.svg", auto_pdf = FALSE)
```
...
```{r, out.height="500px"}
include_graphics("figs/lrt-v-univariate-lod-slides-1.svg", auto_pdf = FALSE)
```
...
- $\uparrow$ Interlocus distance $\longrightarrow$ $\uparrow$ Pleiotropy test statistics

- $\uparrow$ Univariate LOD $\longrightarrow$ $\uparrow$ Pleiotropy test statistics
